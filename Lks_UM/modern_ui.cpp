...
void aim(float l,float top,float w){title(L"COMBAT",tr(L"Assistance à la visée",L"Aim assist"),tr(L"Réglages de ciblage et de comportement.",L"Targeting and behavior settings."));float gap=14,cw=(w-gap)/2;auto a=box(l,top,cw,350),b=box(l+cw+gap,top,cw,350);card(a);card(b);section(tr(L"Ciblage",L"Targeting"),a.left+20,a.top+18,cw-40);
 ...
 const wchar_t* modes[]={tr(L"Ease-out",L"Ease-out"),tr(L"Spring",L"Spring"),tr(L"One Euro",L"One Euro"),tr(L"Hybride",L"Hybrid")};section(tr(L"Mode de lissage",L"Smoothing mode"),b.left+20,b.top+198,cw-40);seg(312,4,std::clamp(g_AimSettings.aimMode,0,3),modes,b.left+12,b.top+224,cw-24,38);
 const wchar_t* targets[]={tr(L"Tête",L"Head"),tr(L"Cou",L"Neck"),tr(L"Poitrine",L"Chest"),tr(L"Bassin",L"Pelvis")};int boneSel=g_AimSettings.aimBone==BONE_NECK?1:g_AimSettings.aimBone==BONE_CHEST?2:g_AimSettings.aimBone==BONE_PELVIS?3:0;section(tr(L"Cible",L"Target"),b.left+20,b.top+272,cw-40);seg(316,4,boneSel,targets,b.left+12,b.top+298,cw-24,38);
}
void esp(float l,float top,float w){title(tr(L"VISUELS",L"VISUALS"),L"ESP",tr(L"Informations claires sur les joueurs et le monde.",L"Clear player and world information."));float gap=12,c1=285,c2=220,c3=w-c1-c2-gap*2;auto a=box(l,top,c1,500),b=box(l+c1+gap,top,c2,500),c=box(l+c1+c2+gap*2,top,c3,500);card(a);card(b);card(c);section(tr(L"Joueurs",L"Players"),a.left+18,a.top+16,c1-36);
 ...
 const wchar_t* boxStyles[]={tr(L"Coin",L"Corner"),tr(L"Normal",L"Normal"),tr(L"Plein",L"Filled"),tr(L"Cercle",L"Circle"),tr(L"3D",L"3D"),tr(L"3D Coin",L"3D Corner"),tr(L"Arrondi",L"Rounded")};section(tr(L"Style de boîte",L"Box style"),c.left+16,c.top+330,c3-32);dropdown(327,boxStyles[std::clamp(g_EspSettings.boxStyle,0,BOX_STYLE_COUNT-1)],boxStyles,BOX_STYLE_COUNT,std::clamp(g_EspSettings.boxStyle,0,BOX_STYLE_COUNT-1),c.left+8,c.top+356,c3-16);
}
...
case 320:return tr(L"Coins 2D : quatre angles pour un cadrage discret.",L"2D corners: four corner marks for a discreet frame.");case 321:return tr(L"Rectangle 2D complet classique.",L"Classic full 2D rectangle.");case 322:return tr(L"Rectangle 2D avec intérieur rempli sombre.",L"2D rectangle with a dark filled interior.");case 323:return tr(L"Cercle centré sur le corps du joueur.",L"Circle centered on the player's body.");case 324:return tr(L"Boîte 3D alignée sur le monde.",L"World-aligned 3D box.");case 325:return tr(L"Coins de la boîte 3D, cadrage léger.",L"3D box corners only, lightweight frame.");case 326:return tr(L"Rectangle 2D aux coins arrondis.",L"2D rectangle with rounded corners.");case 327:return tr(L"Choisit le style de cadre des joueurs.",L"Choose the player frame style.");
...
case 312:return tr(L"Interpolation exponentielle ease-out avec le réglage Ease.",L"Exponential ease-out interpolation driven by the Ease setting.");case 313:return tr(L"Ressort critique amorti : accélération puis décélération naturelle.",L"Critically damped spring: natural acceleration then deceleration.");case 314:return tr(L"Filtre 1€ adaptatif : supprime le bruit sans ajouter de retard.",L"Adaptive 1€ filter: removes noise without adding lag.");case 315:return tr(L"Combiné : le filtre 1€ nettoie la cible, le ressort la poursuit, l'ease-out polit la vélocité.",L"Combined: the 1€ filter cleans the target, the spring tracks it, ease-out polishes the velocity.");case 316:return tr(L"Vise le centre de la tête de la cible.",L"Aim at the center of the target's head.");case 317:return tr(L"Vise le cou de la cible.",L"Aim at the target's neck.");case 318:return tr(L"Vise le torse de la cible.",L"Aim at the target's chest.");case 319:return tr(L"Vise le bassin de la cible.",L"Aim at the target's pelvis.");
