...
struct State {
  ...
  std::vector<Zone> zones; std::vector<std::wstring> configs; int hot{-1}; int down{-1}; float scroll{}; bool french{true};
} s;
...
void aim(float l,float top,float w){title(L"COMBAT",tr(L"Assistance à la visée",L"Aim assist"),tr(L"Réglages de ciblage et de comportement.",L"Targeting and behavior settings."));float gap=14,cw=(w-gap)/2;auto a=box(l,top,cw,350),b=box(l+cw+gap,top,cw,350);card(a);card(b);section(tr(L"Ciblage",L"Targeting"),a.left+20,a.top+18,cw-40);
 ...
 section(tr(L"Précision",L"Precision"),b.left+20,b.top+18,cw-40);slider(303,tr(L"Champ de vision",L"Field of view"),g_AimSettings.aimFov,1.f,30.f,L"°",b.left+12,b.top+54,cw-24);slider(304,tr(L"Lissage",L"Smoothness"),g_AimSettings.aimSmooth,1.f,20.f,L"",b.left+12,b.top+126,cw-24);s.r.text(tr(L"Faites glisser le curseur. Pas décimal de 0,1.",L"Drag the slider. Decimal step: 0.1."),box(b.left+20,b.top+212,cw-40,54),FontCaption,text_dim);
}
void esp(float l,float top,float w){title(tr(L"VISUELS",L"VISUALS"),L"ESP",tr(L"Informations claires sur les joueurs et le monde.",L"Clear player and world information."));float gap=12,c1=285,c2=220,c3=w-c1-c2-gap*2;auto a=box(l,top,c1,440),b=box(l+c1+gap,top,c2,440),c=box(l+c1+c2+gap*2,top,c3,440);card(a);card(b);card(c);section(tr(L"Joueurs",L"Players"),a.left+18,a.top+16,c1-36);
 struct T{int id;const wchar_t*n;const wchar_t*d;bool v;};std::array<T,7> ts={{{105,tr(L"Activer l'ESP",L"Enable ESP"),tr(L"Affichage principal",L"Main display"),g_EspSettings.enabled},{100,tr(L"Boîtes",L"Boxes"),tr(L"Cadre joueur",L"Player frame"),g_EspSettings.box},{101,tr(L"Squelette",L"Skeleton"),tr(L"Structure osseuse",L"Bone structure"),g_EspSettings.skeleton},{107,tr(L"Tête",L"Head"),tr(L"Repère de tête",L"Head marker"),g_EspSettings.headEsp},{102,tr(L"Santé",L"Health"),tr(L"Barre de vie",L"Health bar"),g_EspSettings.health},{103,tr(L"Nom du joueur",L"Player name"),tr(L"Affiche le nom",L"Show player name"),g_EspSettings.name},{104,tr(L"Visibles uniquement",L"Visible only"),tr(L"Filtre de visibilité",L"Visibility filter"),g_EspSettings.visibleOnly}}};for(int i=0;i<7;i++)toggle(ts[i].id,ts[i].n,ts[i].d,ts[i].v,a.left+8,a.top+42+i*54,c1-16);
...
case 112:return tr(L"Affiche un libellé jaune sur chaque poulet détecté dans la carte.",L"Display a yellow label on each chicken detected on the map.");
...
void activate(int id,int x){if(id>=10&&id<=13){s.page=static_cast<Page>(id-10);return;}if(id==20||id==21){s.french=id==20;return;}if(id==30){PostMessageW(s.hwnd,WM_CLOSE,0,0);return;}if(id>=600){size_t i=(size_t)(id-600);if(i<s.configs.size())ModernSelectConfig(s.configs[i]);return;}if(auto p=toggle_ptr(id)){*p=!*p;return;}if(id==90){ShowWindow(s.hwnd,SW_MINIMIZE);return;}if(id==91){PostMessageW(s.hwnd,WM_CLOSE,0,0);return;}if(id==303||id==304){update_slider(id,x);return;}PostMessageW(s.hwnd,WM_COMMAND,MAKEWPARAM(id,BN_CLICKED),(LPARAM)s.hwnd);}
}
...
void mouse_down(int x,int y){s.down=hit((float)x,(float)y);if(s.down==303||s.down==304)update_slider(s.down,x);SetCapture(s.hwnd);InvalidateRect(s.hwnd,nullptr,FALSE);}
