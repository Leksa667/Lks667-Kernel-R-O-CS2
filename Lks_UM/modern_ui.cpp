...
const wchar_t* key_name(int vk){switch(vk){case VK_LBUTTON:return tr(L"Souris 1",L"Mouse 1");case VK_RBUTTON:return tr(L"Souris 2",L"Mouse 2");case VK_MBUTTON:return tr(L"Souris 3",L"Mouse 3");case VK_XBUTTON1:return tr(L"Souris 4",L"Mouse 4");case VK_XBUTTON2:return tr(L"Souris 5",L"Mouse 5");case VK_SHIFT:return tr(L"Maj",L"Shift");case VK_CONTROL:return L"Ctrl";case VK_MENU:return L"Alt";case VK_SPACE:return tr(L"Espace",L"Space");default:break;}static wchar_t b[24];if(vk>=VK_F1&&vk<=VK_F12){swprintf_s(b,L"F%d",vk-VK_F1+1);return b;}if((vk>=L'0'&&vk<=L'9')||(vk>=L'A'&&vk<=L'Z')){b[0]=(wchar_t)vk;b[1]=0;return b;}swprintf_s(b,L"VK %d",vk);return b;}
...
void draw_rail(){
  ...
  zone(29,Nav,box(14,14,214,52));
  s.r.text(L"LKS667 KERNEL RO CS2",box(72,17,154,20),FontCaption,text_hi);s.r.text(tr(L"PANNEAU DE CONTRÔLE",L"CONTROL PANEL"),box(72,39,145,16),FontMicro,accent);
...
void misc(float l,float top,float w){title(tr(L"AFFICHAGE",L"DISPLAY"),tr(L"Options diverses",L"Miscellaneous"),tr(L"Informations complémentaires du moteur actuel.",L"Additional information from the existing engine."));auto c=box(l,top,std::min(w,680.f),250);card(c);section(tr(L"Interface en jeu",L"In-game interface"),c.left+20,c.top+18,c.right-c.left-40);toggle(402,tr(L"Viseur",L"Crosshair"),tr(L"Repère central",L"Center marker"),g_MiscSettings.showCrosshair,c.left+12,c.top+52,c.right-c.left-24);toggle(403,tr(L"Informations / FPS",L"Information / FPS"),tr(L"État de la session",L"Session status"),g_MiscSettings.showGameInfo,c.left+12,c.top+112,c.right-c.left-24);toggle(404,tr(L"Minuteur de bombe",L"Bomb timer"),tr(L"Compte à rebours",L"Countdown"),g_MiscSettings.showBombTimer,c.left+12,c.top+172,c.right-c.left-24);}
...
case 106:return tr(L"Masque les joueurs de votre propre équipe.",L"Hide players on your own team.");case 112:return tr(L"Affiche un libellé jaune sur chaque poulet détecté sur la carte.",L"Display a yellow label on each chicken detected on the map.");case 113:return tr(L"Affiche la barre d'armure (bouclier) à droite du cadre.",L"Display the armor (shield) bar on the right side of the frame.");
...
case 320:return tr(L"Affiche uniquement les joueurs terroristes.",L"Show Terrorist players only.");case 321:return tr(L"Affiche uniquement les joueurs antiterroristes.",L"Show Counter-Terrorist players only.");case 322:return tr(L"Affiche les joueurs des deux équipes.",L"Show players from both teams.");case 327:return tr(L"Choisit le style de cadre des joueurs.",L"Choose the player frame style.");
...
void paint(){HudSetFrench(s.french);render();}
...
void mouse_down(int x,int y){int h=hit((float)x,(float)y);s.down=h;EspLog("[UI] down id=%d x=%d y=%d openList=%d",h,x,y,s.openList);if(s.openList==327&&h!=327&&(h<1000||h>=1000+BOX_STYLE_COUNT))s.openList=-1;if(s.down==303||s.down==304)update_slider(s.down,x);SetCapture(s.hwnd);InvalidateRect(s.hwnd,nullptr,FALSE);}
void mouse_up(int x,int y){int id=hit((float)x,(float)y);EspLog("[UI] up id=%d x=%d y=%d",id,x,y);ReleaseCapture();if(id==s.down&&id>=0)activate(id,x);s.down=-1;InvalidateRect(s.hwnd,nullptr,FALSE);}
void wheel(short){ }
